...
\abstract{Abstract}
High-throughput technologies has open new possibilities to analyse data of repertoires of immune receptors (i.e., T-cell or B-cell receptors). Here we present a manual to an R package \Rpackage{tcR}. %Paper is published in \href{m.pogorely@gmail.com}{Journal of Something}:
%\begin{quote}
